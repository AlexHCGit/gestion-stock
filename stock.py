...
def agregar_repuesto(nombre, descripcion, stock, maquina_id):
    conexion = conectar_db()
    cursor = conexion.cursor()
    cursor.execute('INSERT INTO repuesto (nombre, descripcion, stock, maquina_id) VALUES (?, ?, ?, ?)', 
                   (nombre, descripcion, stock, maquina_id))
    conexion.commit()
    conexion.close()
    ...
    if opcion == "Registrar Entrada":
        ...
        if repuesto_seleccionado == "Nuevo Repuesto":
            ...
            if st.button("Agregar y Registrar Entrada"):
                if nombre_nuevo_repuesto and descripcion_nuevo_repuesto:
                    agregar_repuesto(nombre_nuevo_repuesto, descripcion_nuevo_repuesto, cantidad_nueva, maquina_id)
                    st.success(f"Repuesto '{nombre_nuevo_repuesto}' agregado correctamente y {cantidad_nueva} unidades registradas.")
                else:
                    st.error("Debes completar el nombre y la descripción del repuesto.")
        else:
        ...
        for repuesto in repuestos:
            st.write(f"Repuesto: {repuesto[0]} | Descripción: {repuesto[1]} | Stock: {repuesto[2]} | Máquina: {repuesto[3]} | Hospital: {repuesto[4]}")
